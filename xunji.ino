...
const float KP = 15.0f;            // 比例系数
const float KD = 140.0f;           // 微分系数
const float ALPHA = 0.2f;          // 误差低通滤波系数
const int SEARCH_TURN_SPEED = 200; // 丢线后原地找线速度
const bool ENABLE_REVERSE_TURN = true; // 启用内轮停转转弯（负速改为0速）
const int REVERSE_THRESHOLD = 40;  // 停转启用阈值
    ...
    if (line_seen)
    {
        ...
        if (ENABLE_REVERSE_TURN && abs(correction) > REVERSE_THRESHOLD)
        {
            ...
            if (correction > 0)
            {
                ...
                motor_right = 0;   // 右轮停转（由倒车改为0速）
            }
            else
            {
                // 向左转：右轮快速前进，左轮停转
                motor_left = 0;    // 左轮停转（由倒车改为0速）
